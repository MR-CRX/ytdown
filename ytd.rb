...
logo="""
"""+green+"""Download Video dan Audio YouTube
"""+red+"""By M.R_CRX
"""
...
system("clear")
puts logo+"\n\n"
print ask+"Pilih tipe apa yang mau di download:\n\n1.Audio\n2.Video\n0.Exit\n"+yellow+"---> "+ green
type=gets.chomp
case type
when "1"
...
while not post==""
if status==""
    puts info+"Server eror coba lagi nanti."
    exit    
elsif status == "true"
    break
elsif status == "false"
...
when "2"
print ask+"Masukkan link YouTube:\n"+yellow+"---> "+ green
url=gets.chomp
if not url.include? "youtu"
    puts error+"Error! Ini bukan link YouTube!"
    exit
...
while not post==""
if status==""
    puts "Server eror coba lagi nanti!"
    exit
elsif status== "true"
    break
elsif status == "false"
...
when "0"
    ...
    puts error+"Masukkan salah. Pilih no 1 atau 2"
    exit
end
...
if confirm=="y" && type== "1"
    ...
    puts "\n"+success+"#{title}.mp4 berhasil di download!"
    print "\n"+ask+"Mau pindah directory?(y/n) > "+green
    pt=gets.chomp
    if pt=="y"
        print "\n"+ask+"Masukkan directory > "+green
        path=gets.chomp
        if Dir.exist?(path)
            ...
            puts "\n"+success+"#{title}.mp4 pindah ke #{path}"
        else
            puts error+"Directory tidak ditemukan!"
            exit
        end    
